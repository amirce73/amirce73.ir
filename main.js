...
if (!isMobile) {
    ...
    curs.style.display = "none"; // در موبایل مخفی
}
...
menuBtn.addEventListener("click", () => {
    ...
    if (!flag) {
        gsap.to(".straight-line", {
            width: isMobile ? 300 : 700, // کوتاه‌تر در موبایل
...
menuItems.forEach((item, index) => {
    item.addEventListener("click", () => {
        if (!isMobile) { // انفجار فقط در دسکتاپ
...
function main() {
    ...
    webGLRenderer.setSize(
        isMobile ? window.innerWidth * 0.9 : window.innerWidth,
        isMobile ? window.innerHeight * 0.9 : window.innerHeight
    );
    ...
    let controls = new (function () {
        this.radius = 40;
        this.tube = 17;
        this.radialSegments = isMobile ? 80 : 186;
        this.tubularSegments = isMobile ? 2 : 4;
        this.p = 9;
        this.q = 1;
        this.heightScale = 4;
        this.asParticles = true;
        this.rotate = !isMobile; // چرخش فقط دسکتاپ
    ...
    if (isMobile) gui.close(); // مخفی کردن GUI در موبایل
